/**
 * @fileoverview Main entry file for the Dragora Daemon. This module sets up an
...
app.get('/ftp/info/:id', (req, res) => {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
        if (err) {
            console.error('Error reading file:', Verr);
    ...
    wss.on('connection', (ws, req) => {
        ...
        async function executeCommand(ws, container, command) {
            try {
                ...
                stream.on('end', () => {
                    ws.send('\u001b[1m\u001b[33mcontainer@Dragora~ \u001b[0mServer marked as Offline\r\n'); // Updated to container@Dragora
                });
        ...
        async function performPowerAction(ws, container, action) {
            ...
            if (!actionMap[action]) {
                ws.send(`\r\n\u001b[33m[Dragora] \x1b[0mInvalid action: ${action}\r\n`); // Updated to [Dragora]
                return;
            }

            ws.send(`\r\n\u001b[33m[Dragora] \x1b[0mWorking on ${action}...\r\n`); // Updated to [Dragora]

            try {
                await actionMap[action]();
            } catch (err) {
                console.error(`Error performing ${action} action:`, err);
                ws.send(`\r\n\u001b[33m[Dragora] \x1b[0mAction failed: ${err.message}\r\n`); // Already [Dragora]
            }
        }
...
app.get('/', async (req, res) => {
    try {
        ...
        const response = {
            versionFamily: 1,
            versionRelease: 'dragora ' + config.version,
            online: true,
        ...
        res.status(500).json({ error: 'Docker is not running - it will not function properly.' });
    }
});
...
setTimeout(function () {
    server.listen(port, () => {
        log.info('Dragora is listening on port ' + port);
    });
}, 2000);
